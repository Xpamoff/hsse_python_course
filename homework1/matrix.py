class Tensor:
    def __init__(self, dimension, data):
        if not (isinstance(dimension, int) or isinstance(dimension, tuple)):
            raise ValueError("Dimension must be an int or tuple object")
...
class Matrix(Tensor):
    ...
    def get_row(self, row):
        ...
        return Matrix(tuple([1, self.columns]), data)

    def get_rows(self, rows):
        data = []
        for i in rows:
            data.extend(self.get_row(self.increase_up_to_size(i, self.rows)).data)
        return Matrix(tuple([len(rows), self.columns]), data)

    def transpond(self):
        data = [0 for _ in range(len(self.data))]
        m = Matrix(tuple([self.columns, self.rows]), [])
        for i in range(len(self.data)):
            [r, c] = self.conv_i2rc(i)
            data[m.conv_rc2i(c, r)] = self.data[i]
        return Matrix(tuple([self.columns, self.rows]), data)
    ...
    def __getitem__(self, item):
        ...
        raise ValueError("Index must be an int, list, slice or tuple")
